...
export const COACH_SCHEMA = {
  ...
  properties: {
    reply: { type: "string", description: "Arabic reply shown to the user." },
    actions: {
      type: "array",
      items: {
        anyOf: [
          {
            type: "object",
            additionalProperties: false,
            required: ["type", "name", "kcal", "protein"],
            properties: {
              type: { type: "string", enum: ["log_meal"] },
              name: { type: "string" },
              kcal: { type: "integer" },
          ...
          },
          {
            type: "object",
            additionalProperties: false,
            required: ["type", "routineId"],
            properties: {
              type: { type: "string", enum: ["start_workout"] },
              routineId: {
                type: "string",
                enum: ["reset-2", "wake-2", "night-5", "low-impact-10", "strength-10", "walk-20", "strength-20"],
              },
            },
          },
          {
            ...
            properties: {
              type: { type: "string", enum: ["add_if_then"] },
              when: { type: "string" },
              then: { type: "string" },
            },
          },
